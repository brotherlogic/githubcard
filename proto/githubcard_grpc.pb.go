...
// GithubSubscriberClient is the client API for GithubSubscriber service.
//
// For semantics around ctx use and closing/ending streaming RPCs, please refer to https://pkg.go.dev/google.golang.org/grpc/?tab=doc#ClientConn.NewStream.
type GithubSubscriberClient interface {
	ChangeUpdate(ctx context.Context, in *ChangeUpdateRequest, opts ...grpc.CallOption) (*ChangeUpdateResponse, error)
}

type githubSubscriberClient struct {
	cc grpc.ClientConnInterface
}

func NewGithubSubscriberClient(cc grpc.ClientConnInterface) GithubSubscriberClient {
	return &githubSubscriberClient{cc}
}

func (c *githubSubscriberClient) ChangeUpdate(ctx context.Context, in *ChangeUpdateRequest, opts ...grpc.CallOption) (*ChangeUpdateResponse, error) {
	out := new(ChangeUpdateResponse)
	err := c.cc.Invoke(ctx, "/githubcard.GithubSubscriber/ChangeUpdate", in, out, opts...)
	if err != nil {
		return nil, err
	}
	return out, nil
...
// GithubSubscriberServer is the server API for GithubSubscriber service.
// All implementations should embed UnimplementedGithubSubscriberServer
// for forward compatibility
type GithubSubscriberServer interface {
	ChangeUpdate(context.Context, *ChangeUpdateRequest) (*ChangeUpdateResponse, error)
}

// UnimplementedGithubSubscriberServer should be embedded to have forward compatible implementations.
type UnimplementedGithubSubscriberServer struct {
}

func (UnimplementedGithubSubscriberServer) ChangeUpdate(context.Context, *ChangeUpdateRequest) (*ChangeUpdateResponse, error) {
	return nil, status.Errorf(codes.Unimplemented, "method ChangeUpdate not implemented")
}

// UnsafeGithubSubscriberServer may be embedded to opt out of forward compatibility for this service.
// Use of this interface is not recommended, as added methods to GithubSubscriberServer will
// result in compilation errors.
type UnsafeGithubSubscriberServer interface {
	mustEmbedUnimplementedGithubSubscriberServer()
}

func RegisterGithubSubscriberServer(s grpc.ServiceRegistrar, srv GithubSubscriberServer) {
	s.RegisterService(&GithubSubscriber_ServiceDesc, srv)
}

func _GithubSubscriber_ChangeUpdate_Handler(srv interface{}, ctx context.Context, dec func(interface{}) error, interceptor grpc.UnaryServerInterceptor) (interface{}, error) {
	in := new(ChangeUpdateRequest)
	if err := dec(in); err != nil {
		return nil, err
	}
	if interceptor == nil {
		return srv.(GithubSubscriberServer).ChangeUpdate(ctx, in)
	}
	info := &grpc.UnaryServerInfo{
		Server:     srv,
		FullMethod: "/githubcard.GithubSubscriber/ChangeUpdate",
	}
	handler := func(ctx context.Context, req interface{}) (interface{}, error) {
		return srv.(GithubSubscriberServer).ChangeUpdate(ctx, req.(*ChangeUpdateRequest))
	}
	return interceptor(ctx, in, info, handler)
}

// GithubSubscriber_ServiceDesc is the grpc.ServiceDesc for GithubSubscriber service.
// It's only intended for direct use with grpc.RegisterService,
// and not to be introspected or modified (even as a copy)
var GithubSubscriber_ServiceDesc = grpc.ServiceDesc{
	ServiceName: "githubcard.GithubSubscriber",
	HandlerType: (*GithubSubscriberServer)(nil),
	Methods: []grpc.MethodDesc{
		{
			MethodName: "ChangeUpdate",
			Handler:    _GithubSubscriber_ChangeUpdate_Handler,
		},
	},
	Streams:  []grpc.StreamDesc{},
	Metadata: "githubcard.proto",
